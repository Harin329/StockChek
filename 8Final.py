...
df = pd.read_csv('Exports/MU_Export.csv')
df.head()
...
minmax = MinMaxScaler().fit(df.iloc[:, 4:5].astype('float32')) # 4. close index
df_log = minmax.transform(df.iloc[:, 4:5].astype('float32')) # 4. close index
...
simulation_size = 3 #variance patterns
num_layers = 1
size_layer = 128
timestamp = 5
epoch = 300
dropout_rate = 0.8
test_size = 10 #days ahead
learning_rate = 0.01
...
accepted_results = []
for r in results:
    if (np.array(r[-test_size:]) < np.min(df['4. close'])).sum() == 0 and     (np.array(r[-test_size:]) > np.max(df['4. close']) * 2).sum() == 0:
...
accuracies = [calculate_accuracy(df['4. close'].values, r[:-test_size]) for r in accepted_results]

plt.figure(figsize = (15, 5))
for no, r in enumerate(accepted_results):
    plt.plot(r, label = 'forecast %d'%(no + 1))
plt.plot(df['4. close'], label = 'true trend', c = 'black')
plt.legend()
